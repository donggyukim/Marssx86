...
void ThreadContext::reset() {
  ...
  itlb_cycle = 0;
  icache_cycle = 0;
  commit_interrupt_cycle = 0;
  commit_barrier_cycle = 0;
  commit_stop_cycle = 0;
  is_flushed = 0;
  is_itlb_miss = 0;
  is_l1_icache_miss = 0;
...
#ifndef MULTI_IQ
  int reserved_iq_entries_per_thread = (int)sqrt(ISSUE_QUEUE_SIZE / threadcount);
...
stringbuf& ReorderBufferEntry::get_operand_info(stringbuf& sb, int operand) const {
  ...
  sb << "r", physreg.index();
  if (PHYS_REG_FILE_COUNT > 1) sb << "@", getcore().physregfiles[physreg.rfid].name, physreg.rfid;
