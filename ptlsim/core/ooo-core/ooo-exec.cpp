...
int ReorderBufferEntry::issue() {
  ...
  if (operands[RS] != NULL){
    uop.physreg_rs = operands[RS]->get_tag();
  }
  ...
  /*
  if (fuinfo[uop.opcode].latency > LOADLAT)
	cycles_left = (int) (0.9 * fuinfo[uop.opcode].latency);
  */

  if (config.perfect_long_lat && fuinfo[uop.opcode].latency > 1 && !ld && !st) 
	cycles_left = 1; // by vteori : no long latency functional unit
  uop.fu_lat = cycles_left;
  ...
  if unlikely ((ra.flags | rb.flags | rc.flags) & FLAG_INV) {
    ...
    //  ptl_logfile << "PHYSREG_STATE " << (physreg->state == PHYSREG_WAITING);
    ...
    if likely (physreg->valid()) {
	  ...
	  thread.perfbranchpred.count++;

	  if unlikely (mispredicted) {
	      thread.perfbranchpred.mispredicted++;
	      ...
	      // (Trace)
	      uop.first_branch_mispred = uop.last_branch_mispred ? uop.last_branch_mispred : uop.first_branch_mispred;
  ...
  // for trace
  uop.issue_cycle = sim_cycle;
  if (physreg->state != PHYSREG_WAITING){
    uop.complete_cycle2 = sim_cycle;
  }
  else{
    uop.complete_cycle2 = 0;
  }
...
int ReorderBufferEntry::issuestore(LoadStoreQueueEntry& state, Waddr& origaddr, W64 ra, W64 rb, W64 rc, bool rcready, PTEUpdate& pteupdate) {
  ...
  if(!uop.internal) {
    ...
    if (uop.addrgen_phase){
      uop.addrgen_cycle = sim_cycle;
      uop.addrgen_phase = false;
    }
  ...
  foreach_forward_after (LSQ, lsq, i) {
	...
	uop.redispatch_gen = true;
	thread.redispatch++;
	ptl_logfile << "Trigger : " << *ldbuf.rob << '\n';
	ptl_logfile << "Store : " << *this << '\n';
...
int ReorderBufferEntry::issueload(LoadStoreQueueEntry& state, Waddr& origaddr, W64 ra, W64 rb, W64 rc, PTEUpdate& pteupdate) {
  ...
  foreach_backward_before(LSQ, lsq, i) {
	...
	// Only considered a match if it's not a fence (which doesn't match anything)
	if (stbuf.lfence) 
	  uop.data_deps.push_back(stbuf.rob->physreg->get_tag());
	...
	if (-1 <= sfra_addr_diff && sfra_addr_diff <= 1) {
...
#ifndef SMT_ENABLE_LOAD_HOISTING
	  //load should wait until data of store on which it depends become available. 
	  if (!stbuf.datavalid){
	    sfra = &stbuf;
	  }
#endif
	  ...
	  sfra = &stbuf;
	  //hanhwi
	  uop.addr_deps.push_back(sfra->rob->physreg->get_tag());
	  break;
	}
...
bool OooCore::dcache_wakeup(void *arg) {
  ...
  // ptl_logfile << opinfo[rob.uop.opcode].name << ' ';  
  // ptl_logfile << rob.uop.fetch_cycle << ' ' << rob.uop.issue_cycle << ' ';
  // ptl_logfile << "dcache_wakeup" << sim_cycle << '\n';
  // ptl_logfile << *request << '\n';

  rob.uop.dcache_cycle = sim_cycle;
  rob.uop.request_addr = arg;
  rob.uop.cachelines = request->get_cacheline();
  ...
  if(rob.lsq && request->get_owner_uuid() == rob.uop.uuid &&
	...
	foreach_forward(LSQ, i) {
      ...
      rob.lsq->data = extract_bytes(((byte*)&data) ,
				    sizeshift, signext);
      if (request->get_wakeupid() != -1){
	  rob.uop.wakeup = thread->ROB.distance(request->get_robid(), request->get_wakeupid());
      }
      rob.uop.rob_id = request->get_robid();
      rob.uop.wakeup_id = request->get_wakeupid();
      //      thread->ROB[request->get_wakeupid()].uop.who_wakeup = request->get_robid();
      rob.uop.who_wakeup = request->whowakeup();
      rob.uop.iswakeup = request->get_iswakeup();
      rob.uop.l1_dcache = request->get_miss(0);
      rob.uop.l2_dcache = request->get_miss(1);
...
void ReorderBufferEntry::loadwakeup() {
  //  ptl_logfile << "loadwakeup" << sim_cycle << '\n';
  ...
  if (tlb_walk_level) {
    ...
    if(core->memoryHierarchy->is_l2_dcache_miss(index())){
      getthread().l2_dcache_miss++;
      //      uop.l2_dcache = true;
    }
    if(core->memoryHierarchy->is_l1_dcache_miss(index())){
      getthread().l1_dcache_miss++;
      //      uop.l1_dcache = true;
    }
...
W64 ReorderBufferEntry::annul(bool keep_misspec_uop, bool return_first_annulled_rip) {
  ...
  for (;;) {
    ...
    //  Reset all miss event records in memory hierarchy for the annulled rob
    // -Hanhwi
    thread.core.memoryHierarchy->reset_backend(idx);
    if (annulrob.uop.who_wakeup != -1){
      ReorderBufferEntry& wakeup_rob = ROB[annulrob.uop.who_wakeup];
      if(wakeup_rob.uop.wakeup_id == annulrob.idx){
	wakeup_rob.uop.wakeup_id = -1;
	wakeup_rob.uop.wakeup = 0;
	wakeup_rob.uop.iswakeup = false;
      }
    }
...
void ReorderBufferEntry::redispatch_dependents(bool inclusive) {
  ...
  foreach_forward_from(ROB, this, robidx) {
	...
	if (this != (&reissuerob)){
	  reissuerob.uop.redispatch_gen = false;
	}
	prevrob = &reissuerob;
	/***** (Trace) by vteori *****/
