...
int ReorderBufferEntry::issue() {
  ...
  if likely (ra.nonnull()) {
      ...
      per_physregfile_stats_update(issue.source, ra.rfid, [ra.state]++);
  }
  ...
  if likely ((!uop.rbimm) & (rb.nonnull())) {
      ...
      per_physregfile_stats_update(issue.source, rb.rfid, [rb.state]++);
  }
  ...
  if unlikely ((!uop.rcimm) & (rc.nonnull())) {
      ...
      per_physregfile_stats_update(issue.source, rc.rfid, [rc.state]++);
  }
...
int ReorderBufferEntry::issuestore(LoadStoreQueueEntry& state, Waddr& origaddr, W64 ra, W64 rb, W64 rc, bool rcready, PTEUpdate& pteupdate) {
  ...
  if (!ready) operands[RS] = (sfra) ? sfra->rob->physreg : &core.physregfiles[0][PHYS_REG_NULL];
  ...
  foreach_forward_after (LSQ, lsq, i) {
    ...
    if unlikely ((!ldbuf.store) & ldbuf.addrvalid & ldbuf.rob->issued &
		...
		/***** (Trace) by vteori *****/
		ldbuf.rob->uop.phys_rs = operands[RS]->idx;
  ...
  if unlikely ((contextcount > 1) && (!annul)) {
      ...
      bool lock = core.memoryHierarchy->probe_lock(physaddr, thread.ctx.cpu_index);
...
//#define SMT_ENABLE_LOAD_HOISTING
  ...
  if unlikely (!ready) {
      ...
      if unlikely (sfra && (sfra->lfence | sfra->sfence)) {
  ...
  if unlikely (uop.internal) {
      ...
      load_store_second_phase = 1;
      data = (annul) ? 0 : thread.ctx.loadphys(physaddr, true, sizeshift);
      state.data = data;
      state.datavalid = 1;
      state.invalid = 0;
      state.bytemask = 0xff;
  ...
  */
  ...
  if(rob.lsq && request->get_owner_uuid() == rob.uop.uuid &&
    ...
    if (rob.tlb_walk_level == 0 && (isload(rob.uop.opcode) || isprefetch(rob.uop.opcode))) {
      ...
      rob.lsq->data = extract_bytes(((byte*)&data), sizeshift, signext);
      rob.loadwakeup();
      // for debug by vteori
...
void ReorderBufferEntry::loadwakeup() {
  ...
  if (tlb_walk_level) {
    ...
    physreg->flags &= ~FLAG_WAIT;
#define SKIP_TRANSFER_IN_LOAD
#ifndef SKIP_TRANSFER_IN_LOAD
    physreg->complete();
#endif
    lsq->datavalid = 1;
...
#else
    lsq->datavalid = 1;
	changestate(getthread().rob_issued_list[cluster]);
    cycles_left = 0;
#endif
		
    /***** (Trace) by vteori *****/	
    uop.complete_cycle = sim_cycle;
	// uop.l1_dcache = getcore().memoryHierarchy->is_l1_dcache_miss(index());
	// uop.l2_dcache = getcore().memoryHierarchy->is_l2_dcache_miss(index());
...
W64 ReorderBufferEntry::annul(bool keep_misspec_uop, bool return_first_annulled_rip) {
  ...
  for (;;) {
	...
	/***** by vteori *****/
	if (isload(annulrob.uop.opcode)){
		int idx = annulrob.idx;
