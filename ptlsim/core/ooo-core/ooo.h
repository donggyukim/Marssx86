...
#define LSQ_SIZE (LDQ_SIZE + STQ_SIZE)
  ...
  public:
    ...
    bool ready() const { return state != PHYSREG_WAITING; }
...
#define InitClusteredROBList(name, description, flags)	\
  ...
  struct ThreadContext {
    ...
    W64 commit_interrupt_cycle;
    W64 commit_stop_cycle;
    W64 commit_barrier_cycle;
    ...
    ~ThreadContext(){
      ptl_logfile << "L1 D$ Miss " << l1_dcache_miss << '\n'
		  << "L2 D$ Miss " << l2_dcache_miss << '\n'
		  << "DTLB Miss " << dtlb_miss << '\n'
		  << "L1 I$ Miss " << l1_icache_miss << '\n'
		  << "L2 I$ Miss " << l2_icache_miss << '\n'
		  << "ITLB Miss " << itlb_miss << '\n'
		  << "STORE " << store << '\n'
		  << "LOAD " << load << '\n'
		  << "FENCE " << fence << '\n'
